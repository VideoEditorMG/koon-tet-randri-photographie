      ...
      <div className="container mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-end mb-24 gap-8">
          <div className="max-w-2xl">
            <span className="font-accent text-gold text-xs tracking-[0.4em] mb-4 block uppercase">{t('blog.tag')}</span>
            <h1 className="text-5xl md:text-7xl mb-8 leading-tight">
              {t('blog.title.1')} <span className="italic">{t('blog.title.2')}</span>
            </h1>
          ...
          <div className="flex gap-4">
             {['All', 'Technique', 'Philosophy', 'Travel'].map(cat => (
                <button key={cat} className="font-accent text-[10px] tracking-widest px-4 py-2 border border-white/5 hover:border-gold/30 transition-all uppercase">
                   {cat}
                </button>
             ))}
          </div>
          ...
          {POSTS.map((post, i) => (
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <h3 className="text-2xl md:text-3xl group-hover:text-gold transition-colors duration-500 font-display uppercase">
                  {post.title}
                </h3>
                ...
                <button className="flex items-center gap-3 group/link pt-4">
                   <span className="font-accent text-[10px] font-bold tracking-widest uppercase">{t('blog.read')}</span>
